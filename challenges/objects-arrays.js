...
const universities = [];
graduates.forEach((a) => {
  universities.push(a.university)
})
...
const contactInfo = [];
graduates.forEach((a) => {
  contactInfo.push(`${a.first_name} ${a.email}`)
})
...
const getTotalPopulation = () => {
  let array = []
  zooAnimals.forEach(a => array.push(a.population))
  return populationTotal = array.reduce((acc, curr) => acc + curr)
}
getTotalPopulation()
